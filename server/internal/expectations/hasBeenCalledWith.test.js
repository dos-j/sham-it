const hasBeenCalledWith = require("./hasBeenCalledWith");

describe("unit: hasBeenCalledWith", () => {
  let handler;
  let requestStore;

  beforeEach(() => {
    requestStore = [];
    handler = hasBeenCalledWith(requestStore);
  });
  ...
  test("the error message should match the snapshot", () => {
    ...
    expect(
      handler({ op: "==", prop: "pathname", value: "/test4" })
    ).toMatchSnapshot();
  });
});
